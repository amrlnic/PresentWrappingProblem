...
\begin{center}
		\begin{tabular}{|c|c|}
			...
			\hline
			Rotated X & The boolean that indicates whether a present is rotated or not \\
			\hline
		\end{tabular}
\end{center}
...
Here follows the definition written in pseo-code of what just described:
\begin{algorithm}
	\caption{Retrieve the dimensions of a present}\label{euclid}
	\begin{algorithmic}[1]
		\Procedure{Retrieve Dimension X}{}
		\If {index is \textit{rotated}} \Return \textit{Dimension Y}
		\Else {} \Return \textit{Dimension X}
		\EndIf
		\EndProcedure
	\end{algorithmic}
	\begin{algorithmic}[1]
		\Procedure{Retrieve Dimension Y}{}
		\If {index is \textit{rotated}} \Return \textit{Dimension X}
		\Else {} \Return \textit{Dimension Y}
		\EndIf
		\EndProcedure
	\end{algorithmic}
\end{algorithm}
